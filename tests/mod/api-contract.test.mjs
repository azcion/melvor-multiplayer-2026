...
test('selects v2 or explicit old-backend compatibility without failure-driven downgrades', () => {
	assert.equal(select_api_major(200, { api_versions: [1, 2] }), 2);
	assert.equal(select_api_major(200, { api_versions: [1] }), 1);
	assert.equal(select_api_major(404, null), 1);
	assert.equal(select_api_major(405, null), 1);
	for (const status of [0, 401, 403, 429, 500, 503]) assert.throws(() => select_api_major(status, null));
	for (const body of [{}, { api_versions: [3] }, { api_versions: ['2'] }, { api_versions: [] }])
...
test('resolves logical paths exactly once and preserves query strings', () => {
	...
	assert.equal(api_endpoint('/api/authenticate', 1), '/api/authenticate');
...
test('requires the selected v2 bootstrap contract but accepts older v1 metadata', () => {
	assert.equal(validate_api_bootstrap({}, 1), true);
	assert.equal(validate_api_bootstrap({}, 2), false);
	assert.equal(validate_api_bootstrap({ api_version: 1, api_versions: [1, 2] }, 2), false);
	assert.equal(validate_api_bootstrap({ api_version: 2, api_versions: [1, 2] }, 2), true);
});
