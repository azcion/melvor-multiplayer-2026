...
export function register_equipment_routes(): void {
	async function get_equipment(url: URL, client_id: number, require_guild: boolean): Promise<HandlerResult> {
	...
	session_get_route('/api/guilds/equipment', async (req, url, client_id) => get_equipment(url, client_id, true));
	session_get_route('/api/chat/equipment', async (req, url, client_id) => get_equipment(url, client_id, false));
}
