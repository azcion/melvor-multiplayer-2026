...
const { db, db_execute, db_get_all, db_get_single, guild_membership_exists, legacy_status_activities, parse_player_status_activities, parse_player_status_activity, parse_player_status_skills, session_get_route, session_post_route, status_snapshot_activities, status_snapshot_activity } = runtime;
...
export function register_player_status_routes(): void {
	session_post_route('/api/client/status/sync', async (req, url, client_id, json): Promise<HandlerResult> => {
		...
		if ((!has_skills && !has_activity && !has_activities && !has_gp) || (has_skills && skills === null) ||
			(has_activity && activity === null) || (has_activities && activities === null) || (has_gp && gp === null))
		...
		const save_snapshot = db.transaction(() => {
			...
			if ((has_skills || has_activity || has_activities) && client.status_visible !== 1)
